        ...
        stage('Deploy') {
            container('helm') {
              stage('Helm upgrade') {
                  ...
                  helm upgrade --install --force ${appName} --set-string deployment.image.tag=$(echo $image_tag) --set-string ingress.hosts.host=petclinic.eks.minlab.com -f Helm_Chart/values.yaml Helm_Chart/. --tiller-namespace cicd-tools --namespace cicd-tools
                  '''
              }
            }
        }
